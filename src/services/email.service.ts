      ...
      <div style="text-align: center; padding: 20px; background: #f8f9fa; border-radius: 0 0 10px 10px; border-top: 1px solid #dee2e6;">
        ...
        <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #dee2e6;">
          <p style="margin: 0; color: #adb5bd; font-size: 11px;">
            ...
            📧 <a href="mailto:me@avinashnayak.in" style="color: #0066cc; text-decoration: none;">me@avinashnayak.in</a> | 
...
const formatEmailText = (emailData: EmailPayload): string => {
  ...
  text += `📧 me@avinashnayak.in | 🌐 avinashnayak.in\n\n`;
...
// SendGrid meeting email sending (more reliable for cloud deployments)
const sendMeetingEmailWithSendGrid = async (emailData: {
  recipients: string[];
  subject: string;
  htmlContent: string;
  textContent: string;
  ...
  console.log("=== Using SendGrid for Meeting Email ===");
  sgMail.setApiKey(config.SENDGRID_API_KEY);
  ...
  const msg = {
    ...
    from: {
      email: config.SENDGRID_FROM_EMAIL,
      name: "AI Assistant - Meeting Scheduler",
    },
    subject: emailData.subject,
    ...
    headers: {
      ...
      "List-Unsubscribe": `<mailto:${config.SENDGRID_FROM_EMAIL}?subject=Unsubscribe>`,
      "X-Priority": "3",
      "X-MSMail-Priority": "Normal",
      Importance: "Normal",
    },
    categories: ["meeting-invitation", "ai-assistant"],
    customArgs: {
      meeting_date: emailData.meetingData.dateISO,
      timezone: emailData.meetingData.timezone,
    },
  };

  console.log("SendGrid meeting message prepared:", {
    to: msg.to,
    from: msg.from,
    subject: msg.subject,
  ...
  try {
    const response = await sgMail.sendMultiple(msg);
    console.log(
      "SendGrid meeting email sent successfully:",
      response[0].statusCode
    );
    return {
      messageId:
        response[0].headers["x-message-id"] || "sendgrid-meeting-success",
    };
  } catch (error) {
    console.error("SendGrid meeting email failed:", error);
    throw error;
  }
};
...
export const sendMeetingEmail = async (
  ...
  console.log("=== Meeting Email Service Debug Start ===");
  console.log("Meeting data received:", {
    userEmail: meetingData.userEmail,
    dateISO: meetingData.dateISO,
    endDateISO: meetingData.endDateISO,
  ...
  // Log environment configuration (without sensitive data)
  console.log("Environment config check for meeting email:", {
    hasEmailUser: !!config.EMAIL_USER,
    hasEmailPass: !!config.EMAIL_PASS,
    hasGmailSender: !!config.GMAIL_SENDER,
    hasSendGridApiKey: !!config.SENDGRID_API_KEY,
  ...
  console.log("Meeting email recipients:", recipients);
  ...
  if (config.SENDGRID_API_KEY && config.SENDGRID_FROM_EMAIL) {
    try {
      console.log("Attempting meeting email via SendGrid...");
      const result = await sendMeetingEmailWithSendGrid({
        recipients,
        subject,
        htmlContent,
        textContent,
      ...
      console.log("=== Meeting Email Service Debug End (SendGrid Success) ===");
      return result;
    } catch (sendGridError) {
      console.error(
        "SendGrid failed for meeting email, falling back to SMTP:",
        sendGridError
      );
    }
  } else {
    console.log("SendGrid not configured for meeting email, using SMTP...");
  }

  // Fallback to SMTP
  try {
    console.log("Creating email transporter for meeting email...");
    const transporter = createEmailTransporter();
    console.log("Transporter created successfully for meeting email");

    // Skip verification in production for meeting emails too
    if (config.NODE_ENV === "development") {
      console.log(
        "Verifying SMTP connection for meeting email (development mode only)..."
      );
      try {
        await transporter.verify();
        console.log("SMTP connection verified successfully for meeting email");
      } catch (verifyError) {
        console.warn(
          "SMTP verification failed for meeting email in development:",
          verifyError
        );
        console.log("Proceeding without verification...");
      }
    } else {
      console.log("Skipping SMTP verification for meeting email in production");
    }
    ...
    console.log("Meeting email options prepared:", {
      from: mailOptions.from,
      to: mailOptions.to,
      subject: mailOptions.subject,
    });
    ...
    const result = await transporter.sendMail(mailOptions);
    console.log("Meeting email sent successfully:", result.messageId);
    console.log("=== Meeting Email Service Debug End (SMTP Success) ===");
    return { messageId: result.messageId };
  } catch (error) {
    console.error("=== Meeting Email SMTP Failed ===");
    const err = error as any;
    console.error(
      "Meeting email error type:",
      err.constructor?.name || "Unknown"
    );
    console.error("Meeting email error code:", err.code || "No code");
    console.error("Meeting email error message:", err.message || "No message");
    console.error("Meeting email error stack:", err.stack || "No stack");
    console.log("=== Meeting Email Service Debug End (Failure) ===");
    throw new Error(`Failed to send meeting email: ${err.message}`);
  }
};

// SendGrid contact form email sending (more reliable for cloud deployments)
const sendContactFormEmailWithSendGrid = async (emailData: {
  subject: string;
  htmlContent: string;
  textContent: string;
  ...
  console.log("=== Using SendGrid for Contact Form Email ===");
  sgMail.setApiKey(config.SENDGRID_API_KEY);

  const msg = {
    to: "mail.kunal71@gmail.com", // Portfolio owner
    from: {
      email: config.SENDGRID_FROM_EMAIL,
      name: "Portfolio Contact Form",
    },
    replyTo: {
      email: emailData.contactData.email,
      name: emailData.contactData.fullName,
    },
    subject: emailData.subject,
    text: emailData.textContent,
    html: emailData.htmlContent,
    ...
    categories: ["contact-form", "portfolio"],
    customArgs: {
      contact_email: emailData.contactData.email,
      contact_company: emailData.contactData.company || "N/A",
    },
  };

  console.log("SendGrid contact form message prepared:", {
    to: msg.to,
    from: msg.from,
    replyTo: msg.replyTo,
    subject: msg.subject,
  ...
  try {
    ...
    console.log(
      "SendGrid contact form email sent successfully:",
      response[0].statusCode
    );
    return {
      messageId:
        response[0].headers["x-message-id"] || "sendgrid-contact-success",
    };
  } catch (error) {
    console.error("SendGrid contact form email failed:", error);
    throw error;
  }
};
...
export const sendContactFormEmail = async (
  ...
  console.log("=== Contact Form Email Service Debug Start ===");
  ...
  if (config.SENDGRID_API_KEY && config.SENDGRID_FROM_EMAIL) {
    try {
      console.log("Attempting contact form email via SendGrid...");
      const result = await sendContactFormEmailWithSendGrid({
        subject,
        htmlContent,
        textContent,
        contactData,
      ...
      console.log(
        "=== Contact Form Email Service Debug End (SendGrid Success) ==="
      );
      return result;
    } catch (sendGridError) {
      console.error(
        "SendGrid failed for contact form email, falling back to SMTP:",
        sendGridError
      );
    }
  } else {
    console.log(
      "SendGrid not configured for contact form email, using SMTP..."
    );
  }

  // Fallback to SMTP
  try {
    console.log("Creating email transporter for contact form email...");
    const transporter = createEmailTransporter();
    console.log("Transporter created successfully for contact form email");
    ...
    console.log("Attempting to send contact form email with config:", {
      from: mailOptions.from,
      to: mailOptions.to,
    ...
    console.log("=== Contact Form Email Service Debug End (SMTP Success) ===");
    return { messageId: result.messageId };
  } catch (error) {
    console.error("=== Contact Form Email SMTP Failed ===");
    const err = error as any;
    console.error(
      "Contact form email error type:",
      err.constructor?.name || "Unknown"
    );
    console.error("Contact form email error code:", err.code || "No code");
    console.error(
      "Contact form email error message:",
      err.message || "No message"
    );
    console.error("Contact form email error stack:", err.stack || "No stack");
    console.log("=== Contact Form Email Service Debug End (Failure) ===");
    throw new Error(`Failed to send contact form email: ${err.message}`);
  }
};
